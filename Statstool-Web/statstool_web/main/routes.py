...
def home(mp_id):
    institutions = ["basesave", "renaissance", "colonialism", "printing_press",
                    "global_trade", "manufactories", "enlightenment", "industrialization", "endsave"]
    savegame_dict = {}
    for inst in institutions:
        savegame_dict[inst] = Savegame.query.filter_by(
            mp_id=mp_id, institution=inst).first()
    ...
    if current_save:
        for nation in current_save.player_nations:
            nation_data = NationSavegameData.query.filter_by(
                savegame_id=current_save.id, nation_tag=nation.tag).first()
            if nation_data:
...
def upload_savegames(mp_id=None):
    ...
    if form.validate_on_submit():
        sg_ids = []
        Path(os.path.join(current_app.root_path, current_app.config['UPLOAD_FOLDER'])).mkdir(
        ...
        for file, map_file, name in zip((request.files[form.savegame1.name], request.files[form.savegame2.name]),
            ...
            path = os.path.join(current_app.root_path,
                                current_app.config['UPLOAD_FOLDER'], random)
            file.save(path)

            if map_file:
                map_random = secrets.token_hex(8) + ".png"
                map_path = os.path.join(
                    current_app.root_path, "static/maps", map_random)
            ...
            try:
                ...
                if map_path:
                    savegame = Savegame(
                        file=random, map_file=map_random, owner=current_user, year=year, name=name)
                else:
                    savegame = Savegame(
                        file=random, owner=current_user, year=year, name=name)
                for tag in tag_list:
                    ...
                    nation_data = NationSavegameData(
                        savegame_id=savegame.id, nation_tag=tag, player_name=player_name)
...
def upload_one_savegame(mp_id, institution):
    ...
    if form.validate_on_submit():
        ...
        try:
            player_names_dict, tag_list, year = edit_parse(path)
            if map_path:
                savegame = Savegame(file=random, mp_id=mp_id, map_file=map_random, institution=institution,
                                    owner=current_user, year=year, name=form.savegame_name.data)
            else:
                savegame = Savegame(file=random, mp_id=mp_id, institution=institution,
                                    owner=current_user, year=year, name=form.savegame_name.data)
            for tag in tag_list:
                if not Nation.query.get(tag):
                    nation = Nation(tag=tag)
...
def upload_map(sg_id):
    ...
    if form.validate_on_submit():
        ...
        map_file = request.files[form.map.name]
        map_random = secrets.token_hex(8) + ".png"
        map_path = os.path.join(current_app.root_path,
                                "static/maps", map_random)
        map_file.save(map_path)
...
def account(user_id):
    ...
    header_labels = ["ID", "MP-Name", "Name",
                     "Year", "Filename", "Map", "Delete"]
    mp_header_labels = ["ID", "Name", "Description",
                        "Spielleiter", "Host", "Settings", "Delete"]
    ids = []
    mp_ids = []
    data = []
    mp_names = []
    ...
    return render_template("main/account.html", data=zip(ids, data, maps),
                           ...
                           mp_data=zip(mp_ids, mp_names,
                                       mp_descriptions, mp_gms, mp_hosts),
...
def total_victory_points(mp_id):
    savegame = Savegame.query.filter_by(
        mp_id=mp_id).order_by(desc(Savegame.year)).first()
    current_mp = MP.query.get(mp_id)

    if savegame:
        if mp_id == 3:
            payload = mp3_data(mp_id, savegame)
            return render_template("main/victory_points.html", **payload)

        elif mp_id == 2:
            payload = mp2_data(mp_id, savegame)
            payload["nation_info"] = get_nation_info(savegame)
            return render_template("main/victory_points.html", **payload,)
    ...
    if request.method == "GET":
        for (cat, field) in zip(categories, fields):
            vp = VictoryPoint.query.filter_by(
                mp_id=mp_id, team_id=team_id, category=cat).first()
            if vp:
                if vp.points == 1:
                    field.data = True
            else:
...
def latest_stats(mp_id):
    savegames = Savegame.query.filter_by(
        mp_id=mp_id).order_by(desc(Savegame.year)).all()
    if len(savegames) > 1:
...
def add_team(mp_id):
    form = NewTeamForm()
    if request.method == "POST":
        team = Team(id=form.id.data, mp_id=mp_id,
                    team_tag1=form.select1.data, team_tag2=form.select2.data)
        db.session.add(team)
        db.session.commit()
...
def delete_current_mp_map(mp_id):
    ...
    try:
        os.remove(os.path.join(current_app.root_path,
                  'static/maps', mp.current_map_file))
    except FileNotFoundError:
        pass
